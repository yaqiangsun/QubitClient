...
from resources.lqcs.analysis.inception import s21vsflux
from resources.lqcs.analysis.visualization import plot_s21vsflux
...
def test_s21vsflux(task_key, base_dir):
    for hdf5_path in os.listdir(base_dir):
        ...
        if task_key in "s21vsflux":
            analysis_result = s21vsflux(data)
            fig_list = plot_s21vsflux(data, analysis_result, save_path=f'./tmp/vis/s21vsflux_{pure_name}.png')
            # fig_list[0].show()
            # plt.show(block=True)
...
def main():
    ...
    test_s21vsflux(task_key, base_dir)
