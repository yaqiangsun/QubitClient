...
# -----------配置常量---------------
data_type = 'singleshot_after_read'
...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        I_1 = np.real(value_1) # X 轴：实部
        Q_1 = np.imag(value_1) # Y 轴：虚部
        I_2 = np.real(value_2) # X 轴：实部
        Q_2 = np.imag(value_2) # Y 轴：虚部

        plt.figure(figsize=(6, 6))
        plt.scatter(I_1, Q_1, s=5, alpha=0.6, color='#2E86AB')
        plt.scatter(I_2, Q_2, s=5, alpha=0.6, color="#AB2E2E")

        plt.xlabel('X I Channel (a.u.)')
        plt.ylabel('Y Q Channel (a.u.)')
