...
def send_t12dfit_npy_to_server(dir_path="data/t12dfit", batch_size=5):
    savenamelist = []
    ...
    for start_idx in range(0, total, batch_size):
        ...
        for idx_in_batch, (result, dict_param) in enumerate(zip(results, dict_list)):
            ...
            if result.get('status') == 'failed':
                ...
                continue
            save_path_prefix = f"./tmp/client/result_{TaskName.T12DFIT.value}_{batch_savenames[idx_in_batch]}"
...
def main():
    ...
    send_t12dfit_npy_to_server(base_dir, batch_size=1)
