...
#########################################################################
# Author: yaqiangsun
# Created Time: 2026/03/27 11:18:22
########################################################################
...
def send_delta_npy_to_server(url, api_key, dir_path="data/delta", batch_size=5):
    savenamelist = []
    ...
    for start_idx in range(0, total, batch_size):
        ...
        for idx_in_batch, (result, dict_param) in enumerate(zip(results, dict_list)):
            ...
            save_path_prefix = f"./tmp/client/result_{TaskName.DELTA.value}_{batch_savenames[idx_in_batch]}"
...
def main():
    ...
    base_dir = "tmp/yaqiangsun/qubit_examples/delta"          
    send_delta_npy_to_server(API_URL, API_KEY, base_dir, batch_size=1)
