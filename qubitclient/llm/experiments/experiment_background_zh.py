...
S21VSFLUX = """这是S21 vs Flux实验（腔频 vs 偏置磁通）：我们同时扫描探针频率和施加的磁通偏置电压，以绘制腔共振频率响应。这个二维图揭示了腔共振频率如何随DC偏置（Z控制）变化。成功的结果显示清晰的共振曲线随偏置移动，从而能够校准偏置依赖的腔频率和色散频移。用于优化读取频率和理解串扰。"""
