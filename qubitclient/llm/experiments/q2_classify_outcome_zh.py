...
PROMPT_STANDARD = """强制要求：Reason 对应的解释内容必须全部使用中文进行描述。
...
请按以下格式提供答案：
Classification: <你的中文选项>
Reason: <使用中文写出简短解释>"""

# Special case for qubit_spectroscopy_power_frequency (has background)
PROMPT_QUBIT_SPECTROSCOPY_POWER_FREQUENCY = """强制要求：Reason 对应的解释内容必须全部使用中文进行描述。
...
PROMPT_S21VSFLUX = PROMPT_STANDARD
