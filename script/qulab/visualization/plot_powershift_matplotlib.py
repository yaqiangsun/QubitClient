...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        plt.xlabel('X Drive Power')
        plt.ylabel('Y Probe Frequency')
        plt.xticks(
            ticks=np.arange(0, len(x_axis), 15),
            labels=np.round(x_axis[::15], 15),
