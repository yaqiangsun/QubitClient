...
from resources.quark.anaylsis.inception import singleshot
from resources.quark.anaylsis.visualization import plot_singleshot
        ...
        if task_key.lower() in data["meta"]["name"].lower():
            if len(data["meta"]["other"]["qubits"]) >= 1:
                if task_key in "singleshot":
                    ...
                    fig_list = plot_singleshot(data, analysis_result, save_path=f'./tmp/vis/singleshot_{pure_name}.png')
                    # fig_list[0].show()
                    # plt.show(block=True)
...
def main():
    task_key = "singleshot"
    base_dir = "tmp/data/singleshot"
    test_singleshot(task_key, base_dir)
