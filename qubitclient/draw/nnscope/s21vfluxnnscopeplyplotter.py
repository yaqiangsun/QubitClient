            ...
            if (index % 2 != 0):
                ...
                for i in range(len(points_list)):
                    ...
                    self.add_line(fig, x=xy_x,
                                  y=xy_y, row=(index // cols) + 1, col=(index % cols) + 1, name=f'{class_id}:{round(confidence_list[index//2][i], 2)}', color_index=i,
                                  line_style_index=0)
                    centcol = len(xy_y)//2
                    self.add_annotation(fig, x=xy_x[centcol],
                                        y=xy_y[centcol],
                                        text=f'{class_id}:{round(confidence_list[index//2][i], 2)}', row=(index // cols) + 1, col=(index % cols) + 1)
