...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        # ---------解析数据---------
        x = each_qubit_info[0]
        points_value = each_qubit_info[1]
        points_1 = points_value[0]
        points_2 = points_value[1]

        x_axis = each_qubit_info[2]
        line_value = each_qubit_info[3]
        line_1 = line_value[0]
        line_2 = line_value[1]

        # ----------绘制---------
        plt.figure(figsize=(6, 4))
        plt.plot(x, points_1, label='Data 1')
        plt.plot(x, points_2, label='Data 2')
        
        plt.plot(x_axis, line_1, label='Fit 0')
        plt.plot(x_axis, line_2, label='Fit 1')

        plt.xlabel('X DRAG Derivative Amplitude (a.u.)')
        plt.ylabel('Y AC Stark Detuning Error (MHz)')
