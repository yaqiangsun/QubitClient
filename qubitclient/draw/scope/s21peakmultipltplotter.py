...
class S21PeakMultiDataPltPlotter(QuantumDataPltPlotter):
    ...
    def plot_result_npy(self, **kwargs):
        ...
        for i in range(n_plots):
            ...
            # 绘制曲线
            line1, _ = self.add_line(ax, x, y1, color_index=0,line_style_index=0)

            y2_range = np.max(y2) - np.min(y2)
            offset = y2_range   # 使用第一条曲线范围的100%作为偏移

            line2, _ = self.add_line(ax2, x, y2+offset, color_index=1,line_style_index=0)
