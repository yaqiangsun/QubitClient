...
"""pi_pulse_half scan pipeline with UI storage & cmd args
Usage:
    1. Start UI server first: qubitclient ui start
    2. Example:
        python -m skills.qcontrol-qubit-calib.scripts.pipeline.rabihalf_pipeline
...
from analysis.update import rabihalf_update
...
def parse_args():
    parser = argparse.ArgumentParser(description="PUPULSE_HALF Measurement Pipeline (UI storage sync enabled)")
    parser.add_argument("--qubits", "-q", type=str, nargs="+", default=["q1ld5"],
                        help="Target qubit list, default: q1ld5")
    parser.add_argument("--piamp-half-start", "-ps", type=float, default=0, help="Pulse amplitude start")
    parser.add_argument("--piamp-half-end", "-pe", type=float, default=5, help="Pulse amplitude end")
    parser.add_argument("--piamp-half-samples", "-pn", type=int, default=100, help="Amplitude sample count")
    parser.add_argument("--pi-len-half", "-pl", type=float, default=60, help="Pulse Length")
...
def get_rabihalf_hdf5_res(args):
    store = PipelineResultStore(backend=StorageBackend.LOCAL)
    task_name = CtrlTaskName.RABIHALF.value
    qubit_name_list = args.qubits
    save_folder = args.save_folder
    qname = qubit_name_list[0]

    qubit_ctrl_client = QubitCtrlClient()
    pi_amp_half_star_original = qubit_ctrl_client.query_param(qname=qname, key="pi_amp_half_star")
    pi_len_half_star_original = qubit_ctrl_client.query_param(qname=qname, key="pi_len_half_star")

    try:
        # 1.采集数据
        set_params = {
            "qubits": qubit_name_list,
            "piamp_half_start": args.piamp_half_start,
            "piamp_half_end": args.piamp_half_end,
            "piamp_half_sample_num": args.piamp_half_samples,
            "pi_amp_half_star": pi_amp_half_star_original,
            "pi_len_half_star": pi_len_half_star_original
        }
        ...
        logging.info(f"[RABIHALF] Task started run_id={run_id[:8]}")

        data_id = qubit_ctrl_client.run(
            CtrlTaskName.RABIHALF,
            qubits=qubit_name_list,
            piamp_half_start=args.piamp_half_start,
            piamp_half_end=args.piamp_half_end,
            piamp_half_sample_num=args.piamp_half_samples,
            pi_len_half=args.pi_len_half
        )
        ...
        # 3.绘图
        img_save_path = f'{save_folder}/{CtrlTaskName.RABIHALF.value}_{qname}_{run_id}.png'
        ...
        if args.update:
            # 调用更新接口
            update_amp_map = pi_pulse_half_update(
            ...
            if qname in update_amp_map:
                target_pigate_amp_half_star = float(update_amp_map[qname])

                target_values = [target_pigate_amp_half_star]


                qubit_ctrl_client.update_param(
                    qname=qname,
                    task_type=CtrlTaskName.RABIHALF,
                    values=[target_values]
                )

                new_full_params["pi_amp_half_star"] = target_pigate_amp_half_star
...
if __name__ == '__main__':
    cli_args = parse_args()
    get_rabihalf_hdf5_res(cli_args)