...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        print("len(y_axis), len(x_axis): ",len(y_axis), len(x_axis))

        img_height = len(y_axis)
        img_width = len(x_axis)

        # ---------初始化图像，并逐行逐列填充数据---------
        img = np.zeros((len(x_axis), len(y_axis)), dtype=np.float32)
        for row_idx in range(img_width):
            # print("\n", row_idx)

            cur_row_value = []
            for col_idx in range(img_height):
                x = x_axis[col_idx]
        ...
        plt.xlabel('X Flux Bias')
        plt.ylabel('Y Readout Amplitude')
        plt.xticks(
            ticks=np.arange(0, len(x_axis), 4),
            labels=np.round(x_axis[::4], 4),
            
        )
        # plt.yticks(ticks=np.arange(0, len(y_axis), 4), labels=np.round(y_axis[::10] * 1e6, 2) )
