...
S21PEAKMULTI 是 Scope 中的一个任务，用于检测峰值并给出峰值置信度。
...
| task_type | TaskName | 是 | 任务类型，固定为`TaskName.S21PEAKMULTI` |
...
x:  np.ndarray,shape(A,),表示频率信息
amp:  np.ndarray,shape(A,),表示幅度信息
phi:  np.ndarray,shape(A,),表示相位信息

每个量子比特对应一个键（如 "Q0"），值为 (x, amp,phi) 
#### 调用示例

```python
# 使用文件路径

response = client.request(file_list=["data/singlepath/file1.npy", "data/singlepath/file2.npy"], task_type=TaskName.S21PEAKMULTI)
...
response = client.request(file_list=dict_list, task_type=TaskName.S21PEAKMULTI)
...
[
  {
    "peaks": List[List[int]],          // 峰值位置
    "confs": List[List[float]],        // 峰值置信度
    "freqs_list": List[List[float]],   // 峰值横坐标
    "status": str       // 处理状态
  },
  ...
]
...
for idx, (result, dict_param) in enumerate(zip(results, dict_list)):
  save_path_prefix = f"./tmp/client/result_{TaskName.S21PEAKMULTI.value}_{savenamelist[idx]}"
  save_path_png = save_path_prefix + ".png"
  save_path_html = save_path_prefix + ".html"
  plt_plot_manager.plot_quantum_data(
      data_type='npy',
      task_type=TaskName.S21PEAKMULTI.value,
