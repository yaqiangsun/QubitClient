...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        # ---------解析数据---------
        info_0 = each_qubit_info[0]
        info_1 = each_qubit_info[1]
        info_2 = each_qubit_info[2]

        x_axis = np.arange(0, len(info_0[0]))

        plt.figure(figsize=(6, 4))

        for each_info in [info_0, info_1, info_2]:
            for each_line in each_info:
                plt.plot(x_axis, each_line, label='Data')

        plt.xlabel('X Readout Power')
        plt.ylabel('Y P(1>)')
