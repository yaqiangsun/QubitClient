...
from resources.quark.analysis.inception import spectrum
from resources.quark.analysis.visualization import plot_spectrum
        ...
        if "spectrum" in data["meta"]["name"].lower():
            if len(data["meta"]["other"].get("qubits", [])) >= 1:
                ...
                if task_key in "spectrum":
                    ...
                    fig_list = plot_spectrum(data, analysis_result, save_path=f'./tmp/vis/spectrum_{pure_name}_allqubits.png')
                    # fig_list[0].show()
                    # plt.show(block=True)
