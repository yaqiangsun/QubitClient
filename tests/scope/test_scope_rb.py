...
def send_rabicos_npy_to_server(dir_path="data/rb", batch_size=5):
    savenamelist = []
    ...
    for start_idx in range(0, total, batch_size):
        ...
        threshold = -1
        results = client.get_result(response, threshold=threshold, task_type=TaskName.RB.value)
        ...
        for idx_in_batch, (result, dict_param) in enumerate(zip(results, dict_list)):
            ...
            save_path_prefix = f"./tmp/client/result_{TaskName.RB.value}_{batch_savenames[idx_in_batch]}"
...
def main():
    base_dir = "tmp/yaqiangsun/qubit_examples/rb"
    send_rabicos_npy_to_server(base_dir, batch_size=1)
