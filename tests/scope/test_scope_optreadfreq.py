...
def send_optreadfreq_npy_to_server(url, api_key, dir_path="data/optreadfreq", batch_size=5):
    savenamelist = []
    ...
    for start_idx in range(0, total, batch_size):
        ...
        for idx_in_batch, (result, dict_param) in enumerate(zip(results, dict_list)):
            ...
            save_path_prefix = f"./tmp/client/result_{TaskName.OPTREADFREQ.value}_{batch_savenames[idx_in_batch]}"
...
def main():
    ...
    base_dir = "tmp/optreadfreq"
    send_optreadfreq_npy_to_server(API_URL, API_KEY, base_dir, batch_size=1)
