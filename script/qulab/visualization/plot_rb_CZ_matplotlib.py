...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        # ---------解析数据---------
        value_1 = each_qubit_info[0]
        # same structure 6
        value_2 = each_qubit_info[1]
        ...
        data5 = value_1[4]
        data6 = value_1[5]
        print("len(data1), len(data2), len(data3): ", len(data1), len(data2), len(data3))
        print("len(data4), len(data5), len(data6): ", len(data4), data5, len(data6))
        # print("len 2: ", data4, data5, data6)
        ...
        plt.scatter(data1, data2, label='Data 1', color = 'red')
        # plt.plot(data1, data3, label='Data 3', color='black') # 可能是置信度，不用画
        plt.plot(data1, data4, label='Data 2', color='orange')

        plt.scatter(data1_2, data2_2, label='Data 1_2', color = 'green')
        plt.plot(data1_2, data4_2, label='Data 2_2', color='black')

        plt.xlabel('X Sequence Length')
        plt.ylabel('Y P(0>)')
