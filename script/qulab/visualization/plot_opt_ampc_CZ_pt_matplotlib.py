...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        # ---------解析数据---------
        x_axis = each_qubit_info[0]
        value = each_qubit_info[1]

        value_2 = each_qubit_info[2] # sometimes None, or list[ 3 array ]
        ...
        if type(value[0]) == np.ndarray:
            ...
            plt.plot(x_axis, line_1, label='Data 1', color = 'black')
            plt.plot(x_axis, line_2, label='Data 2', color = 'orange')
            plt.plot(x_axis, line_3, label='Data 3', color = 'green')

        # elif type(value[0]) == float:
        #     plt.plot(x_axis, value, label='Data 1', color = 'red')

        plt.xlabel('X Pulse Duration Time')
        plt.ylabel('Y Readout Amplitude')
