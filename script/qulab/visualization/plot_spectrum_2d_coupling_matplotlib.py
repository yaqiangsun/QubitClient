...
# -----------配置常量---------------
data_type = 'spectrum_2d_coupling'
...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        plt.xlabel('X Pulse Frequency')
        plt.ylabel('Y Pulse Amplitude')
        plt.xticks(
            ticks=np.arange(0, len(x_axis), 5),
            labels=np.round(x_axis[::5], 5),
            
        )
        plt.yticks(ticks=np.arange(0, len(y_axis), 50), labels=np.round(y_axis[::50] * 1e6, 2) )
