...
T2FIT 是 Scope 中的一个任务，用于对量子比特的进行 **高斯指数衰减 + 余弦振荡** 拟合，返回每个量子比特的拟合参数、拟合曲线、 拟合优度及处理状态

$$
y = A \cdot e^{-(x/T2)^2 - x/T1/2} \cdot \cos(2\pi w x + \phi) + B
$$
...
| `task_type` | `TaskName`                                | 是   | 任务类型，固定为 `TaskName.T2FIT`                                    |                             |
...
# 或过滤后的结果
threshold = 0.5
results = client.get_result(response, threshold=threshold, task_type=TaskName.T2FIT.value)
```
...
```python
{
  ...
  "results": [
    {
      "params_list": [
        [0.4920083111862632, 0.6714043624192048, 1548609.489409118, 
         2.4673768788997257e-07, 46379065.63362402, 0.9226591650285556]
      ],
      "r2_list": [0.9597461983867187],
      "fit_data_list": [
...
for idx, (result, dict_param) in enumerate(zip(results, dict_list)):
  save_path_prefix = f"./tmp/client/result_{TaskName.T2FIT.value}_{savenamelist[idx]}"
