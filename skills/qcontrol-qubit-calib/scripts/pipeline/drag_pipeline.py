...
"""Real drag measurement pipeline, write data to storage for web UI real-time display
Usage:
...
from analysis.update import drag_update
...
def get_drag_res(args):
    ...
    try:
        ...
        print(f"[drag] Task started run_id={run_id[:8]}")
        ...
        # =========== 分析 ============
        analysis_result = drag(raw_data)

        # =========== 绘制波形图==========
        img_save_path = f'{save_folder}/{CtrlTaskName.DRAG.value}_{qubit_name_list[0]}_{run_id}.png'
 
        plot_drag(raw_data, analysis_result, save_path=img_save_path)
