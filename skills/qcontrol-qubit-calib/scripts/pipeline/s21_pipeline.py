...
Usage:
    ...
    2. cmd params example:
            python -m skills.qcontrol-qubit-calib.scripts.pipeline.s21_pipeline
...
def parse_args():
    ...
    # 不填则从硬件查询
    parser.add_argument("--readout_freq_star", "-f", type=float, default=None,
                        help="Manual readout_freq_star (GHz). Auto query hardware if not set.")
    # 半带宽
...
def get_s21_res(args):
    ...
    try:
        ...
        if args.readout_freq_star is not None:
            readout_freq_star = args.readout_freq_star
        else:
        ...
        set_params = {
            ...
            "readout_freq_star": readout_freq_star
        }
        ...
        data_id = qubit_ctrl_client.run(
            CtrlTaskName.S21,
            qubits=qubit_name_list,
            frequency_center=set_params["readout_freq_star"],
