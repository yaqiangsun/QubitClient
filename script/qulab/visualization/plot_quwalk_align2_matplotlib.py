...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        print("len(ele1), len(ele2), len(ele3), len(ele4): ", \
              len(ele1), len(ele2), len(ele3), len(ele4))


        plt.figure(figsize=(6, 4))


        plt.plot(x_axis, ele1, label='data 1', color = 'red')
        plt.plot(x_axis, ele2, label='data 2', color = 'orange')
        plt.plot(x_axis, ele3, label='data 3', color = 'blue')
        plt.plot(x_axis, ele4, label='data 4', color = 'black')

        plt.xlabel('X Readout Delay')
        plt.ylabel('Y P(1>)')
