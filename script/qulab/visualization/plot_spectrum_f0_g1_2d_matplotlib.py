...
def plot_pic(file_path):
    ...
    for each_qubit_name, each_qubit_info  in image.items():
        ...
        if len(x_axis) != col_num: # may swap
            y_axis = each_qubit_info[1]
            x_axis = each_qubit_info[2]
        print("len(y_axis)==11, len(x_axis)==51: ",len(y_axis), len(x_axis))
        ...
        plt.xlabel('X Pulse Amplitude')
        plt.ylabel('Y Pulse Duration (ns)')
        plt.xticks(
            ticks=np.arange(0, len(x_axis), 10),
            labels=np.round(x_axis[::10], 3),
            
        )
        plt.yticks(ticks=np.arange(0, len(y_axis), 30), labels=np.round(y_axis[::30] * 1e6, 2) )
