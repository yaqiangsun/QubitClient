...
"""T1 measurement pipeline, write data to storage for web UI real-time display
Usage:
    1. Start UI server first: qubitclient ui start
    2. cmd params example: (-dn must be bigger than 10)
            python -m skills.lqcs-qubit-calib.scripts.pipeline.t1_pipeline -q q1ld5
...
def parse_args():
    parser = argparse.ArgumentParser(description="T1Measurement Pipeline (UI storage sync enabled)")
    # 被测比特列表
    ...
    # 延时终止
    parser.add_argument("--delay-end", "-de", type=int, default=1000,
                        help="Delay end value, default 1000")
    # 延时采样点数
    parser.add_argument("--delay-sample-num", "-dn", type=int, default=17,
                        help="Delay sampling count, default 17")
    # 偏置
    parser.add_argument("--zpa", "-z", type=float, default=0.0,
                        help="zpa")
...
def get_t1_hdf5_res(args):
    store = PipelineResultStore(backend=StorageBackend.LOCAL)
    task_name = CtrlTaskName.T1.value
    ...
    try:
        ...
        # 数据分析
        analysis_result = t1fit(raw_data)
        logging.info("analysis: %s", analysis_result)

        # 绘图
        pure_name = qubit_name_list[0]
        img_save_path = f'{save_folder}/{CtrlTaskName.T1.value}_{pure_name}_{run_id}.png'
        fig_list = plot_t1fit(raw_data, analysis_result, save_path=img_save_path)
