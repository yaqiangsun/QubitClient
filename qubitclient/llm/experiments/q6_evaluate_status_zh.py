...
"""
Q6: 评估状态任务 (中文版)
判断实验成功/失败状态并给出建议，所有推理、备注文字全部使用中文书写
"""
# ========== 独立 Prompt 字符串定义 ==========
PROMPT_COUPLER_FLUX = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
Notes: <使用中文写出1-3句推理过程>"""

PROMPT_CZ_BENCHMARKING = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_DRAG = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_GMM = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
- SUCCESS: 两个明显分离的聚类
- NO_SIGNAL: 无法区分的聚类
...
PROMPT_MICROWAVE_RAMSEY = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_MOT_LOADING = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
- SUCCESS: 定义明确、对称的高斯形状的云，高信噪比
...
PROMPT_PINCHOFF = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_PINGPONG = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
- SUCCESS: 信号随门计数增加而稳定
...
PROMPT_QUBIT_FLUX_SPECTROSCOPY = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_QUBIT_SPECTROSCOPY = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_QUBIT_SPECTROSCOPY_POWER_FREQUENCY = """强制要求：所有推理、备注说明文字必须全部使用中文。
...
PROMPT_RABI = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
- SUCCESS: 清晰振荡，拟合追踪数据
- FIT_POOR: 振荡可见但拟合偏离
...
PROMPT_RABI_HW = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
- SUCCESS: 清晰振荡，拟合追踪数据
- FIT_POOR: 振荡可见但拟合偏离
...
PROMPT_RAMSEY_CHARGE_TOMOGRAPHY = """强制要求：所有推理、备注说明文字必须全部使用中文。
分类此扫描<image>中观察到的电荷跳跃活动。
分类标准
...
PROMPT_RAMSEY_FREQ_CAL = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_RAMSEY_T2STAR = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
- SUCCESS: 清晰振荡带衰减包络，拟合追踪数据
- NO_DETUNING: 信号平坦——无振荡
...
PROMPT_RES_SPEC = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
- SUCCESS: 可见的清晰共振特征
...
PROMPT_RYDBERG_RAMSEY = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_RYDBERG_SPECTROSCOPY = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_T1 = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
- SUCCESS: 清晰的指数衰减，拟合追踪数据
- NO_SIGNAL: 布居数平坦——量子比特从未被激发或瞬时衰减
...
PROMPT_T1_FLUCTUATIONS = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
- STABLE: T1值紧密围绕单个基线聚集，变化由测量散点主导
- TELEGRAPHIC: T1在两个或多个离散亚稳态之间突变切换——表示耦合到双能级系统缺陷
...
PROMPT_TWEEZER_ARRAY = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
# ========== Not in QCalEval ==========
PROMPT_S21 = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_RABICOS = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_RAMSEY = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_S21VSFLUX = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_POWERSHIFT = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_SPECTRUM_2D = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_OPTPIPULSE = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
...
PROMPT_RB = """强制要求：所有推理、备注说明文字必须全部使用中文。
评估图像<image>并确定实验状态。
决策标准
