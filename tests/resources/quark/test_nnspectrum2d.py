...
from resources.quark.anaylsis.inception import nnspectrum2d
from resources.quark.anaylsis.visualization import plot_nnspectrum2d
        ...
        if task_key.lower() in data["meta"]["name"].lower():
            if len(data["meta"]["other"]["qubits"]) >= 1:
                if task_key in "nnspectrum2d":
                    ...
                    fig_list = plot_nnspectrum2d(data, analysis_result, save_path=f'./tmp/vis/nnspectrum2d_{pure_name}.png')
                    # fig_list[0].show()
                    # plt.show(block=True)
