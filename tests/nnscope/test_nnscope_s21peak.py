...
def send_s21peak_npy_to_server(url, api_key, dir_path="data/33137"):
    # get all file in dir
    savenamelist=[]
    ...
    for idx, (result, dict_param) in enumerate(zip(results, dict_list)):
        save_path_prefix = f"./tmp/client/result_{NNTaskName.S21PEAK.value}_{savenamelist[idx]}"
...
def main():
    ...
    base_dir = "tmp/yaqiangsun/qubit_examples/s21peak"
    send_s21peak_npy_to_server(API_URL, API_KEY, base_dir)
